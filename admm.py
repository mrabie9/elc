    ...
    if epoch != 1 and (epoch - 1) % args.admm_epochs == 0 and batch_idx == 0:
        with torch.no_grad():
            for i, (name, W) in enumerate(model.named_parameters()):
                ...
                current_u = ADMM.ADMM_U[name]
                detached_w = W.detach()
                ADMM.ADMM_Z[name] = detached_w + current_u  # Z(k+1) = W(k+1)+U[k]
                _, updated_Z = weight_pruning(args, ADMM.ADMM_Z[name], ADMM.prune_ratios[name])  # equivalent to Euclidean Projection
                ADMM.ADMM_Z[name] = updated_Z.detach()
                if (args.verbose):
                    if writer:
                        writer.add_scalar('layer:{} W(k+1)-Z(k+1)'.format(name), torch.sqrt(torch.sum((detached_w - ADMM.ADMM_Z[name]) ** 2)).item(), epoch)
                        writer.add_scalar('layer:{} Z(k+1)-Z(k)'.format(name), torch.sqrt(torch.sum((ADMM.ADMM_Z[name] - Z_prev) ** 2)).item(), epoch)

                ADMM.ADMM_U[name] = (detached_w - ADMM.ADMM_Z[name] + current_u).detach()  # U(k+1) = W(k+1) - Z(k+1) +U(k)
                check_nan(detached_w, f'W_{name}')
    ...
    if epoch != 1 and (epoch - 1) % args.mask_admm_epochs == 0 and batch_idx == 0:
        with torch.no_grad():
            for i, (name, M) in enumerate(model.named_parameters()):
                if 'mask' not in name:
                    continue
                current_k = ADMM.ADMM_K[name]
                detached_m = M.detach()
                ADMM.ADMM_Y[name] = detached_m + current_k  # Z(k+1) = W(k+1)+U[k]
                ADMM.ADMM_Y[name] = mask_pruning(args.adaptive_ratio, ADMM.ADMM_Y[name], args.mask[name.replace('w_mask', 'weight')]).detach() 
                ADMM.ADMM_K[name] = (detached_m - ADMM.ADMM_Y[name] + current_k).detach()  # U(k+1) = W(k+1) - Z(k+1) +U(k)
                check_nan(detached_m, f'M_{name}')
...
def append_admm_loss(args, ADMM, model, ce_loss):
    ...
    for k, v in admm_loss.items():
        ...
        prev_loss = mixed_loss# mixed_loss.clone()
        mixed_loss += v

...
def _get_backbone(
    ...
    if getattr(args, "multi_head", False) and n_tasks > 1 and (num_classes % n_tasks == 0):
        per_task = num_classes // n_tasks
        classes_per_task = [per_task] * n_tasks
...
class DisjointBayesianClassifier(nn.Module):
    ...
    def __init__(self, input_size: int, num_classes: int, n_tasks: int,
        ...
        self.n_tasks = n_tasks
        self.num_classes = num_classes
        self.classes_per_task = num_classes // n_tasks
        ...
        self.task_slices: List[Tuple[int, int]] = [
            (task * self.classes_per_task, (task + 1) * self.classes_per_task)
...
class Net(nn.Module):
    ...
    def __init__(self, input_size: int, num_classes: int, args: object) -> None:
        ...
        assert num_classes % n_tasks == 0, "UCL expects balanced class splits per task"
        self.args = args
        self.input_size = input_size
        self.num_classes = num_classes
        self.n_tasks = n_tasks
        self.nc_per_task = num_classes//n_tasks

        self.model = DisjointBayesianClassifier(input_size, num_classes, n_tasks, self.cfg, args)
        ...
        backbone_params = [
            param
            for name, param in self.model.feature_net.named_parameters()
...
class DisjointNet(nn.Module):
    """UCL learner with a single Bayesian head that allocates disjoint neurons per task."""
    ...
    def __init__(self, input_size: int, num_classes: int, args: object) -> None:
        ...
        self.current_task: Optional[int] = None
        self.model_old: Optional[DisjointBayesianClassifier] = None
        self.saved = False
        self.ce = nn.CrossEntropyLoss(reduction="mean")
        self.is_task_incremental: bool = True
    ...
    def _snapshot_model(self) -> DisjointBayesianClassifier:
        clone = DisjointBayesianClassifier(
            self.input_size,
            self.num_classes,
            self.n_tasks,
            self.cfg,
